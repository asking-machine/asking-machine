...
def options_and_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Asking-Machine thinks for you about the meanings.",
        epilog="Example:  asking-machine input_text.txt > output_text.txt"
...
def run():
    """
    $ text | asking-machine                        # Accepts text from the pipe
    $ echo "...<text>..." | asking-machine         #

    $ asking-machine multilogue.txt new_turn.txt    # ...or files.
    """
    ...
    try:
        ...
        syslog.openlog(
            ident="asking-machine",
            logoption=syslog.LOG_NDELAY,
            facility=syslog.LOG_USER
        )
        # Signal (single line less than 4096 only!)
        syslog.syslog(syslog.LOG_INFO, f"asking-machine: {utterance}.")
        syslog.closelog()
